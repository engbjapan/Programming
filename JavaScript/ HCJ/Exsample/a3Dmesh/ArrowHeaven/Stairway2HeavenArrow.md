...
天国への階段(*Stairway to Heaven*)では？と言う思いに至った事を私なりにまとめたものです。
...
まず、「`.~`チェーンスタイル」とは、複数のジェネレーター関数を連結し、一連の値を生成することができる機能を指しています。
...
#### 無名class new(`.~`)チェーンに於けるメソッド呼び出しコード例
